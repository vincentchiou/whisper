...
- 連線成功後可直接用下拉選單切換模型
- 若是 `LM Studio`，下拉選單一改就要自動切換並載入模型，不可只記住名稱
- 真正生成 `進階SEO.txt` 前，也要再次確認 `LM Studio` 已載入目前所選模型
- 若 `LM Studio` 的思考型模型第一輪只回推理內容，要自動重試一次並要求只輸出最終答案
- `LM Studio` 若要穩定產出完整 `進階SEO.txt`，優先選一般對話 / instruct 類模型，例如 `gemma-4-e4b-it`
- `qwen ... reasoning / thinking` 這類模型目前可連線、可切換、可短回覆，但完整 `進階SEO.txt` 仍可能失敗
- 若模型漏掉第 3 段或第 4 段，系統必須自動回填 hashtags 與章節目錄，不可讓下載檔留空
- 支援：
  - `Ollama`
  - `LM Studio`
  - `Groq`
